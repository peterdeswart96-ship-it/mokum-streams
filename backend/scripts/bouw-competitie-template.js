...
async function bouw() {
  ...
  // .info is het tekstvak links. De renderer (thumbnailHtml.js) verkleint .team-regels als
  ...
  const css = [
    ...
    '.logo{position:absolute;left:56px;top:36px;height:150px;z-index:3;filter:drop-shadow(0 6px 18px rgba(0,0,0,.6))}',
    '.info{position:absolute;left:56px;top:200px;bottom:130px;width:510px;display:flex;flex-direction:column;justify-content:center;z-index:2;overflow:hidden}',
    '.niveau{font-size:40px;white-space:nowrap;overflow:hidden;color:#ff2b2b;text-transform:uppercase;letter-spacing:5px;line-height:1.1;margin-bottom:6px}',
    ...
    '.vs{font-size:52px;color:#ff2b2b;line-height:1;margin:8px 0 6px;letter-spacing:2px}',
  ...
  const html = `<!DOCTYPE html><html lang="nl"><head><meta charset="utf-8"><style>${antonFontFaces()}</style><style>${css}</style></head>`
    + `<body><div class="canvas"><img class="bg" src="${achtergrond}"><img class="logo" src="${logo}">`
    + '<div class="info"><div class="niveau">{{NIVEAU}}</div><div class="team">{{THUISTEAM}}</div>'
